...
export class SongSingerTunesService {
  ...
  private async loadUsersByIds(ids: number[]): Promise<Map<number, UserMini>> {
    const uniq = Array.from(
      new Set(ids.filter((x) => Number.isFinite(x) && x > 0)),
    );
  ...
  private async requireViewerUserIdByEmail(
    viewerEmail: string,
  ): Promise<number> {
  ...
  async upsertSingerTuneInternal(
    songId: number,
    viewerEmail: string,
    input: PutSingerTuneInput,
  ) {
  ...
  async deleteSingerTuneInternal(
    songId: number,
    viewerEmail: string,
    rowId?: string,
  ) {
