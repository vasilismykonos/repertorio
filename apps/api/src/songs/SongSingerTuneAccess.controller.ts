...
function requireInternalKey(req: any) {
  ...
  if (!expected) {
    ...
    throw new UnauthorizedException("Server misconfigured (INTERNAL_API_KEY missing)");
  }

  const got = (req?.headers?.["x-internal-key"] ?? req?.headers?.["X-Internal-Key"]) as string | undefined;
  if (!got || got !== expected) {
