...
export class ElasticSongsController {
   ...
   */
  private readonly STATUS_FIELD =
    process.env.ES_STATUS_FIELD ?? 'status.keyword';
   ...
   */
  private sanitizePhraseTokens(tokens: string[]): {
    head: string[];
    last: string | null;
  } {
    const toks = (tokens ?? [])
   ...
   */
  private buildStrictSpanPhrase(
    field: string,
    tokens: string[],
    boost: number,
    opts?: { minLastPrefixLen?: number },
  ) {
  ...
  private async esSearch<T>(body: any): Promise<EsSearchResponse<T>> {
    ...
    if (!res.ok) {
      throw new Error(
        `ES _search failed HTTP ${res.status} ${res.statusText} – body: ${text.slice(0, 1200)}`,
      );
    }
  ...
  private parsePositiveIntOrNull(
    v?: string,
    min = 1,
    max = 9999,
  ): number | null {
  ...
  async search(
    ...
    try {
      ...
      const categoryIds = this.parseIdsFromAliases(
        categoryIdsStr,
        categoryIdLegacy,
        categoryIdAlt,
      );
      ...
      const lyricistIds = this.parseIdsFromAliases(
        lyricistIdsStr,
        lyricistIdAlt,
      );
      ...
      const singerFrontId =
        this.parseNumber(singerFrontIdStr, 0, 0, 2_000_000) || null;
      const singerBackId =
        this.parseNumber(singerBackIdStr, 0, 0, 2_000_000) || null;
      ...
      const years = Array.from(
        new Set<number>(this.parseIdList(yearsStr) ?? []),
      );
      ...
      if (statusList?.length) {
        if (statusList.length === 1)
          filters.push({ term: { [this.STATUS_FIELD]: statusList[0] } });
      ...
      if (yearFrom !== null || yearTo !== null) {
        const must: any[] = [];
        if (yearFrom !== null)
          must.push({ range: { maxYear: { gte: yearFrom } } });
        if (yearTo !== null) must.push({ range: { minYear: { lte: yearTo } } });
        filters.push({ bool: { must } });
      }

      if (composerIds?.length)
        filters.push({ terms: { composerId: composerIds } });
      if (lyricistIds?.length)
        filters.push({ terms: { lyricistId: lyricistIds } });
        ...
        filters.push({
          bool: {
            should: composerList.map((c) => ({
              bool: {
                should: [
                  { term: { 'composerName.keyword': c } },
                  { match: { composerName: c } },
                ],
        ...
        filters.push({
          bool: {
            should: lyricistList.map((c) => ({
              bool: {
                should: [
                  { term: { 'lyricistName.keyword': c } },
                  { match: { lyricistName: c } },
                ],
                minimum_should_match: 1,
              },
            })),
      ...
      if (singerFrontIds.length || singerBackIds.length) {
        ...
        if (singerFrontIds.length === 1)
          nestedMust.push({
            term: { 'versionSingerPairs.frontId': singerFrontIds[0] },
          });
        else if (singerFrontIds.length > 1)
          nestedMust.push({
            terms: { 'versionSingerPairs.frontId': singerFrontIds },
          });

        if (singerBackIds.length === 1)
          nestedMust.push({
            term: { 'versionSingerPairs.backId': singerBackIds[0] },
          });
        else if (singerBackIds.length > 1)
          nestedMust.push({
            terms: { 'versionSingerPairs.backId': singerBackIds },
          });
      ...
      if (hasQuery) {
        ...
        const qTitle = this.buildStrictSpanPhrase('title', normTokens, 30, {
          minLastPrefixLen: 1,
        });
        if (qTitle) should.push(qTitle);

        // First lyrics: strong
        const qFirst = this.buildStrictSpanPhrase(
          'firstLyrics',
          normTokens,
          15,
          { minLastPrefixLen: 1 },
        ...
        const qLyrics = this.buildStrictSpanPhrase('lyrics', normTokens, 3, {
          minLastPrefixLen: 3,
        });
      ...
      const body = {
        ...
        aggs: {
          ...
          createdById: {
            ...
            aggs: {
              topName: {
                top_hits: { size: 1, _source: { includes: ['createdByName'] } },
              },
            },
          },
          ...
          composerId: {
            ...
            aggs: {
              topName: {
                top_hits: { size: 1, _source: { includes: ['composerName'] } },
              },
            },
          },
          lyricistId: {
            ...
            aggs: {
              topName: {
                top_hits: { size: 1, _source: { includes: ['lyricistName'] } },
              },
            },
          },
              ...
              byId: {
                ...
                aggs: {
                  topName: {
                    top_hits: {
                      size: 1,
                      _source: { includes: ['versionSingerPairs.frontName'] },
                    },
                  },
                },
              },
              ...
              byId: {
                ...
                aggs: {
                  topName: {
                    top_hits: {
                      size: 1,
                      _source: { includes: ['versionSingerPairs.backName'] },
                    },
                  },
                },
              },
