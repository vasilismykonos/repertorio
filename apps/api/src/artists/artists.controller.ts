...
function normalizeRoles(
  roleRaw?: string | string[],
): VersionArtistRoleString[] {
...
export class ArtistsController {
  ...
  async createArtist(@Body() body: CreateArtistBody) {
    ...
    const rawTitle = String((body as any).title ?? '')
      .trim()
      .replace(/\s+/g, ' ');
    ...
    if (!rawTitle && !firstNameVal && !lastNameVal) {
      throw new BadRequestException('title or firstName/lastName is required');
    }
    ...
    return this.artistsService.createArtistFull(
      {
        ...
        firstName:
          'firstName' in body
            ? normalizeNullableText((body as any).firstName)
            : null,
        lastName:
          'lastName' in body
            ? normalizeNullableText((body as any).lastName)
            : null,
        ...
        wikiUrl:
          'wikiUrl' in body
            ? normalizeNullableText((body as any).wikiUrl)
            : null,
        biography:
          'biography' in body
            ? normalizeNullableText((body as any).biography)
            : null,

        imageUrl:
          'imageUrl' in body
            ? normalizeNullableText((body as any).imageUrl)
            : null,
      },
      file,
    );
  ...
  async updateArtistFull(
    ...
    const bornYear =
      'bornYear' in body
        ? parseNullableNumber((body as any).bornYear)
        : undefined;
    const dieYear =
      'dieYear' in body
        ? parseNullableNumber((body as any).dieYear)
        : undefined;
    ...
    return this.artistsService.updateArtistFull(
      id,
      {
        title:
          'title' in body
            ? normalizeNullableText((body as any).title)
            : undefined,
        firstName:
          'firstName' in body
            ? normalizeNullableText((body as any).firstName)
        ...
        sex:
          'sex' in body ? normalizeNullableText((body as any).sex) : undefined,
        bornYear,
        dieYear,
